...
-- 2. 这里不直接写 MySQL，而是用 XADD 写入 Redis Stream，让后台消费者异步落库：
--    入口请求只承接高并发资格判断，数据库写压力被 MQ 削峰。
-- 3. 返回码约定：
...
-- 3.2. 判断用户是否已经下单。
-- SISMEMBER 是 O(1) 判断；高并发入口先在 Redis 拦住重复请求，减少数据库唯一性/查询压力。
...
--
-- 这一步成功后，Java 请求线程就可以返回 orderId；后台消费者通过 XREADGROUP 拉取消息、落库、XACK。
