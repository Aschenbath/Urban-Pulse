 ...
 * 2. Redis Lua 脚本负责“库存判断 + 一人一单判断 + 扣 Redis 库存 + 写 MQ”这一组原子操作。
 ...
 * - BlockingQueue 只在当前 JVM 内存里，应用重启消息就丢。
 * - 多实例部署时，每个 JVM 都有自己的队列，无法天然协调。
 * - 没有 pending-list / ack 语义，消费者处理到一半宕机时不好补偿。
 *
 * 为什么这里先用 Redis Stream 而不是 RabbitMQ：
 * - Redis 实战项目项目已经依赖 Redis，Stream 可以在不增加外部中间件的前提下讲清 MQ 核心概念。
 * - Stream 有消息 ID、消费者组、ACK、pending-list，足够覆盖面试常问的“异步削峰”和“失败补偿”。
 * - 真正生产环境如果需要更强的路由、延迟、死信、管理能力，可以迁移到 RabbitMQ/RocketMQ/Kafka。
 */
@Slf4j
@Service
...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    /**
     * 服务启动后初始化消费者组并启动后台消费者。
     *
     * XGROUP CREATE stream.orders g1 $ MKSTREAM:
     * - stream.orders：订单消息队列名。
     * - g1：消费者组名。一个组里的多个消费者可以分摊消息。
     * - $：从当前最新消息之后开始消费，避免服务重启后把历史已处理消息全扫一遍。
    ...
    /**
     * 秒杀入口：只做 Redis 原子校验 + 入 MQ，然后快速返回订单 ID。
     *
     * 注意这里订单 ID 只生成一次：
    ...
    private class VoucherOrderHandler implements Runnable {
         ...
         * - 下次消费者恢复后，用 ReadOffset.from("0") 读取本消费者组未 ACK 的老消息。
         * - 重试成功后再 ACK。
         *
         * 面试重点：MQ 不是“放进去就完事”，要讲清楚 ACK 和失败补偿，否则异步下单会丢单。
         */
    ...
    public void createVoucherOrder(VoucherOrder voucherOrder) {
        ...
        if (!success) {
             ...
             */
            throw new IllegalStateException("数据库扣减库存失败，可能库存已不足或 Redis/MySQL 库存不一致，voucherId=" + voucherId);
        }
